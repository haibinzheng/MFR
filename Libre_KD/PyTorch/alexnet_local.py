...
if __name__ == "__main__":
    FGSM_IMAGES_PATH = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/image/selected_2/military/AlexNet/FGSM"  # 测试数据集路径

    ALEXNET_MODEL_PATH = '/data/Newdisk/chenjingwen/DT_B4/GD_detect/image/clean_models/AlexNet_Military.pth'
    ALEXNET_BNN_MODEL_PATH = '/data/Newdisk/chenjingwen/DT_B4/GD_detect/image/kd_save/save_bnn/AlexNet_bnn_military.pth'
    ALEXNET_KDE_PATH = '/data/Newdisk/chenjingwen/DT_B4/GD_detect/image/kd_save/save_kdes/kdes_military_AlexNet.joblib'
    ALEXNET_CLASSIFIER_PATH = '/data/Newdisk/chenjingwen/DT_B4/GD_detect/image/kd_save/save_rf/rf_AlexNet_ALL_military.pkl'
    ALEXNET_SCALER_PATH = '/data/Newdisk/chenjingwen/DT_B4/GD_detect/image/kd_save/save_scaler/scaler_AlexNet_ALL_military.pkl'
    model = []

    ALEXNET_BNN_MODEL = model_load(ALEXNET_BNN_MODEL_PATH, 'bnn')
    ALEXNET_KDE = model_load(ALEXNET_KDE_PATH, 'kde')
    ALEXNET_CLASSIFIER = model_load(ALEXNET_CLASSIFIER_PATH, 'classifier')
    ALEXNET_SCALER = model_load(ALEXNET_SCALER_PATH, 'scaler')
    ALEXNET_MODEL = model_load(ALEXNET_MODEL_PATH, 'ori')
    model.append(ALEXNET_BNN_MODEL)
    model.append(ALEXNET_KDE)
    model.append(ALEXNET_CLASSIFIER)
    model.append(ALEXNET_SCALER)
        ...
        for file in files:
            if file.lower().endswith(supported_extensions):
                ...
                with open(file_path, "rb") as img_file:
                    ...
                    result = detection(img_file, model)
                    end_time = time.time()
