...
def get_torch_models(Model):
    if Model == 'AlexNet':
        ...
        model.load_state_dict(torch.load('kd_save/clean_models/AlexNet_imagenet_100.pth'))
    if Model == 'GoogleNet':
...
def get_bayesian_uncertainty(Model, input):
    ...
    bnn_modelpath = 'kd_save/save_bnn/{}_bnn.pth'.format(Model)
...
def get_train_test_data(params, csv_path):
    ...
    scaler_path = 'kd_save/save_scaler/scaler_{}_{}_onlykd.pkl'.format(Model, attack_method)
    ...
    #                                                     test_size=0.2,
    #                                                     random_state=42)
    X_train, X_test, y_train, y_test = kds_2, kds_2, Y, Y
    print(kds_2)
    print(Y)
    # print("使用三者结合进行预测：")
    model_filename = 'kd_save/save_rf/rf_{}_{}_onlykd.pkl'.format(Model, attack_method)
    # model_filename = 'save_rf/rf_{}_{}_finetune.pkl'.format(Model, attack_method)
    ...
    # adv_saved = 0
    # ori_saved = 0
    # correct_samples_folder = '/data/Newdisk/chenjingwen/DT_B4/GD_detect/image_mindspore/dataset/PatchAttack_less_less'  # 你可以指定新的文件夹路径
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    print((end_time - start_time + 198.479) / 100)
    print(result)
