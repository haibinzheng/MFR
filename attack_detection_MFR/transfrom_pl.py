...
for dududu in ['00']:
    print(dududu)
    x_test=np.load('/tmp/fgsm_resnet152_imagenet/image_all/all_2000/x_test{}'.format(dududu)+'.npy')
    x_adtest=np.load('/tmp/fgsm_resnet152_imagenet/image_all/all_2000/x_adtest{}'.format(dududu)+'.npy')
    ...
    for j in [-2,-1,1,2]:
        ...
        print(np.min(x_rotest))
        name=str(25*j)
        np.save('/tmp/fgsm_resnet152_imagenet/image_all/all_2000/x_test{}'.format(dududu)+'_{}'.format(name), x_rotest)
        ...
        print(np.min(x_adrotest))
        name=str(25*j)
        np.save('/tmp/fgsm_resnet152_imagenet/image_all/all_2000/x_adtest{}'.format(dududu)+'_{}'.format(name), x_adrotest)
