...
def detect_main(detparams):
    ...
    test_image_dir = '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/YOLOX_adv_image_true/FGSM'
        ...
        for root, dirs, files in os.walk(test_image_dir):  # 以该数据集为例，可替换
            ...
            for file in files:
                if file.lower().endswith(supported_extensions):
                    ...
                    print(file_path)
                    transformed_image = yolox_detection(file_path, Method, detector, yolos)
                    ...
                    if 'ori' in file_path.split('/'):
                        ...
                        # 转换为 PIL 图像并保存
                        img = Image.fromarray((transformed_image_np * 255).astype(np.uint8))
                        img.save('/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/yolo_detect/temp/adv/{}.png'.format(os.path.splitext(file)[0]))
