...
def crop_and_save_image(image_path, box, conf, save_path):
    ...
    # cropped_image = TF.crop(Image.fromarray(image), a2, a1, b2, b1)
    # cropped_image = TF.crop(Image.fromarray(image), a1, a2, b1, b2)
...
def train_binary_classifier(params):
    ...
    image_SaveDir = "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/warship_frcnn/DetectData"
    # for i in range(len(attack_methods)):
    #     os.makedirs(image_SaveDir + "/{}/train/adv".format(attack_methods[i]), exist_ok=True)
    #     os.makedirs(image_SaveDir + "/{}/test/adv".format(attack_methods[i]), exist_ok=True)
    #     os.makedirs(image_SaveDir + "/{}/train/ori".format(attack_methods[i]), exist_ok=True)
    #     os.makedirs(image_SaveDir + "/{}/test/ori".format(attack_methods[i]), exist_ok=True)
    #     os.makedirs(image_SaveDir + "/DetectData/{}/ori".format(attack_methods[i]), exist_ok=True)
    #     os.makedirs(image_SaveDir + "/DetectData/{}/adv".format(attack_methods[i]), exist_ok=True)
    ...
    # detect_path = []
    # detect_ground_label = []
    pickNumber_v1 = 4000  # 2500 白盒攻击正常样本2000张，'FGSM','IGSM','MIFGSM','PGD'','PSO_patch'各500张
    ...
    for i in range(len(All_Attack_Methods)):
        ...
        if All_Attack_Methods[i] == 'backdoor':
            adv_path = '../datasets/FLIR_Attack/backdoor/data'
        else:
            adv_path = '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/warship_frcnn/patch_result'
    ...
    for i in range(num):
        ...
        print(f"***************1.2训练对抗数据检测*************************")
        _, outputs_post_adv, output_conf_adv = frcnn.detect_image(imageAdv_pil)
        ...
        conf2 = output_conf_adv[box_idx_adv]

        print(f"***************3.训练图片裁剪和保存*************************")
        # 图片裁剪和保存
        crop_and_save_image(train_path[num + i], box1, conf1,
                            os.path.join("/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/warship_frcnn/DetectData/{}/train/ori".format(attack_methods[0]),
                                         os.path.basename(train_path[num + i])))
        crop_and_save_image(train_path[i], box2, conf2,
                            os.path.join("/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/warship_frcnn/DetectData/{}/train/adv".format(attack_methods[0]),
                                         os.path.basename(train_path[i])))
        # exit()
    logger.info(f"训练样本数：{num}")
    time.sleep(10)

    ### 源正常数据集路径：../datasets/COCO/images/val2017，源对抗数据集路径：../datasets/YOLOX_adv_image_true
    ...
    for i in range(num111):
        ...
        print(f"***************1.测试数据检测*************************")
        print(f"***************1.1测试原始数据检测*************************")
        _, outputs_post_ori, output_conf_ori = frcnn.detect_image(imageOri_pil_test)
        print(f"***************1.2测试对抗数据检测*************************")
        _, outputs_post_adv, output_conf_adv = frcnn.detect_image(imageAdv_pil_test)
        ...
        print(f"***************3.测试图片裁剪和保存*************************")
        # 图片裁剪和保存
        crop_and_save_image(test_path[num111 + i], box1, conf1,
                            os.path.join("/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/warship_frcnn/DetectData/{}/test/ori".format(attack_methods[0]),
                                         os.path.basename(test_path[num111 + i])))
        crop_and_save_image(test_path[i], box2, conf2,
                            os.path.join("/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/warship_frcnn/DetectData/{}/test/adv".format(attack_methods[0]),
    ...
    path111 = "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/warship_frcnn/DetectData/{}/train".format(attack_methods[0])
    path222 = "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/warship_frcnn/DetectData/{}/test".format(attack_methods[0])
    ...
    time.sleep(10)
    #
    # logger.info(f"****开始训练二分类模型*******")
    #
    # # transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    #
    # # Initialize the dataset
    # dataset = datasets.ImageFolder(root=image_SaveDir + '/{}/train'.format(attack_methods[0]), transform=transform)
    # test_dataset = datasets.ImageFolder(root=image_SaveDir + '/{}/test'.format(attack_methods[0]),
    #                                     transform=transform)
    #
    ...
    #     if Test_Accuarcy > Best_Acc and Test_Accuarcy > 0.60:
    #         Best_Acc = Test_Accuarcy
    #         logger.info(f"成功保存！！！")
    #         saved_flag = True
    #         torch.save(model.state_dict(),
    #                    '../datasets/frcnn_zz/{}_CNN_Classifier_{}.pth'.format(
    #                        attack_methods[0], Test_Accuarcy))
    #         saved_path = '../datasets/frcnn_zz/{}_CNN_Classifier_{}.pth'.format(
    #             attack_methods[0], Test_Accuarcy)
    logger.info(f"All_Attack_Methods={attack_methods[0]}")
    jsontext = {
...
def CNN_Classifier_Detect(params):
    ...
    # attack_methods = ['FGSM', 'IGSM', 'MIFGSM', 'PGD', 'PSO_patch', 'GuassianBlurAttack', 'backdoor']
    attack_methods = params["Attack_Methods"]
    image_SaveDir = "../datasets/frcnn_zz/Dataset_FRCNN"
    for i in range(len(attack_methods)):
        os.makedirs(image_SaveDir + "/DetectData/{}/ori".format(attack_methods[i]), exist_ok=True)
        os.makedirs(image_SaveDir + "/DetectData/{}/adv".format(attack_methods[i]), exist_ok=True)
    ...
    advImage_number = 0
    detect_result = []
    # adv_path='../datasets/YOLOX_adv_image_true/{}'.format(All_Attack_Methods[i])
    if Attack_Method == 'backdoor':
        adv_path = '../datasets/FLIR_Attack/backdoor/data'
    else:
        adv_path = "../datasets/FILR_adv_true_mwb/{}".format(Attack_Method)
    ...
    ###读正常样本
    ori_path = '../datasets/FILR_RGB_image_mwb/FGSM'
    if os.path.isdir(ori_path):
        ori_file = get_image_list(ori_path)
    else:
        ori_file = [ori_path]
    ...
    for i in range(num222):
        ...
        _, outputs_post_ori, outputs_conf_ori = frcnn.detect_image(imageOri_pil_detect)
        _, outputs_post_adv, outputs_conf_adv = frcnn.detect_image(imageAdv_pil_detect)
        ...
        # 图片裁剪和保存
        crop_and_save_image(detect_path[num222 + i], box1, conf1,
                            os.path.join(image_SaveDir + "/DetectData/{}/ori".format(Attack_Method),
                                         os.path.basename(detect_path[num222 + i])))
        crop_and_save_image(detect_path[i], box2, conf2,
                            os.path.join(image_SaveDir + "/DetectData/{}/adv".format(Attack_Method),
                                         os.path.basename(detect_path[i])))
    logger.info(f"测试样本数：{num222}")

    # Initialize the dataset
    test_dataset = datasets.ImageFolder(root=image_SaveDir + '/DetectData/{}'.format(Attack_Method),
                                        transform=transform)
    ...
    if Attack_Method == 'FGSM':
        model_path = '../datasets/frcnn_zz/FGSM_CNN_Classifier_1.0.pth'
    elif Attack_Method == 'IGSM':
        model_path = '../datasets/frcnn_zz/IGSM_CNN_Classifier_0.9945945945945946.pth'
    elif Attack_Method == 'MIFGSN':
        model_path = '../datasets/frcnn_zz/MIFGSM_CNN_Classifier_0.7195767195767195.pth'
    elif Attack_Method == 'GuassianBlurAttack':
        model_path = '../datasets/frcnn_zz/GuassianBlurAttack_CNN_Classifier_0.9673202614379085.pth'
    elif Attack_Method == 'PGD':
        model_path = '../datasets/frcnn_zz/PGD_CNN_Classifier_0.558974358974359.pth'
    elif Attack_Method == 'PSO_patch':
        model_path = '../datasets/frcnn_zz/PSO_patch_CNN_Classifier_0.8005181347150259.pth'

    elif Attack_Method == 'backdoor':
        model_path = '../datasets/frcnn_zz/backdoor_CNN_Classifier_0.7819148936170213.pth'
    ...
    path333 = image_SaveDir + "/DetectData/{}".format(Attack_Method)
...
if __name__ == "__main__":
    params = {
        ...
        "Attack_Methods": ["patch_attack"]  # {FGSM、IGSM、GuassianBlurAttack、PSO_patch
    }
