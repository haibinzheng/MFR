...
if __name__ == "__main__":
    ...
    FGSM_IMAGES_PATH = "/data/Newdisk/chenjingwen/DT_B4/IndicatorsCoverage/SJS/PrePro_Plus/YOLOX/dataset/COCO/backdoor_less"  # 测试数据集路径
    DETECTOR_PATH = '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/yolo_detect/model_data/backdoor_CNN_classifier.pth'
    ...
    for root, dirs, files in os.walk(FGSM_IMAGES_PATH):  # 以该数据集为例，可替换
        ...
        for file in files:
            if file.lower().endswith(supported_extensions):
                ...
                #         '/data/Newdisk/chenjingwen/DT_B4/IndicatorsCoverage/SJS/PrePro_Plus/YOLOX/dataset/COCO/backdoor_less/ori',
                #         os.path.basename(file)))
                # # elif'ori' in file_path.split('/') and result == 0:
                # #     if random.random() < 0.05:
                # #         shutil.copy(file_path, os.path.join(
                # #             '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/YOLOX_adv_image_true/select/UAP/ori',
                # #             os.path.basename(file)))
                # #     print("良性判断错了")
                # if 'poi' in file_path.split('/') and result == 0 and adv_saved < 50:
                #     adv_saved += 1
                #     shutil.copy(file_path, os.path.join(
                #         '/data/Newdisk/chenjingwen/DT_B4/IndicatorsCoverage/SJS/PrePro_Plus/YOLOX/dataset/COCO/backdoor_less/poi',
                #         os.path.basename(file)))
                # # elif 'adv' in file_path.split('/') and result == 1:
                # #     if random.random() < 0.05:
                # #         shutil.copy(file_path, os.path.join(
                # #             '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/YOLOX_adv_image_true/select/UAP/adv',
                # #             os.path.basename(file)))
                # #     print("恶意判断错了")
