...
def train_binary_classifier(params, predictor):
    ...
    for i in range(len(all_attack_methods)):
        os.makedirs(
            "../datasets/yolo_zz/{}/train/adv".format(all_attack_methods[i]),
            exist_ok=True)
        os.makedirs(
            "../datasets/yolo_zz/{}/test/adv".format(all_attack_methods[i]),
            exist_ok=True)
        os.makedirs(
            "../datasets/yolo_zz/{}/train/ori".format(all_attack_methods[i]),
            exist_ok=True)
        os.makedirs(
            "../datasets/yolo_zz/{}/test/ori".format(all_attack_methods[i]),
            exist_ok=True)
        os.makedirs("../datasets/DetectData/ori", exist_ok=True)
        os.makedirs("../datasets/DetectData/adv", exist_ok=True)
    ...
    test_path = []
    test_ground_label = []
    pickNumber_v1 = 3600  # 2500 白盒攻击正常样本2000张，'FGSM','IGSM','MIFGSM','PGD'','PSO_patch'各500张
    ...
    All_Attack_Methods = all_attack_methods
    # All_Attack_Methods=['mosaic']
    # train_adv_path=os.path.join(ori_save_path,'train','adv')
    # test_adv_path=os.path.join(ori_save_path,'test','adv')
    # os.makedirs(train_adv_path,exist_ok=True)
    # os.makedirs(test_adv_path,exist_ok=True)
    # path111=os.path.join(ori_save_path,'test')
    # path222=os.path.join(ori_save_path,'train')
    for i in range(len(All_Attack_Methods)):
        advImage_number = 0
        ...
        if All_Attack_Methods[i] == 'backdoor':
            adv_path = '../datasets/COCO/val2017/backdoor'
        elif All_Attack_Methods[i] == 'ALL_Attack_warship':
            ...
            # adv_path = "/data0/BigPlatform/DT_Project/001-Dataset/VOC_adv_image_true/{}".format(All_Attack_Methods[i])
    ...
    ori_path = os.path.join(
        '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/YOLOX_warship_adv/all_attack/ori')
    ...
    for i in range(len(adv_file)):
        ...
        if image_name in ori_file:
            image_files.append(image_name)
            oriImage_number += 1
            if oriImage_number < 2800 + 1:
                train_path.append(image_name)
                train_ground_label.append(0)
                number3 += 1
            if oriImage_number > 2800:
                test_path.append(image_name)
    ...
    for i in range(num):
        ...
        outputs_adv, outputs_post_adv, img_info_adv, outputs_conf_adv = predictor.inference(train_path[i])
        outputs_ori, outputs_post_ori, img_info_ori, outputs_conf_ori = predictor.inference(train_path[num + i])

        if outputs_post_adv[0] is None or outputs_post_ori[0] is None:
        ...
        for box_adv in outputs_post_adv[0]:  # 每个框都保存，然后选框最大的一个
            for idx, box_ori in enumerate(outputs_post_ori[0]):
        ...
        for j in range(len(max_box_idx)):
            max_box_distance = max_boxDistance(outputs_post_ori[0][max_box_idx[j]][0:4])
            if max_box_distance > max_distance:
                max_distance = max_box_distance
                box_idx_ori = max_box_idx[j]
                box_idx_adv = j
        box1 = outputs_post_ori[0][box_idx_ori][0:4]
        conf1 = outputs_conf_ori[0][box_idx_ori]
        # print(box1)
        # print(conf1)
        # print(conf1)
        # print(conf1.shape)
        # exit()
        box2 = outputs_post_adv[0][box_idx_adv][0:4]
        conf2 = outputs_conf_adv[0][box_idx_adv]
        ...
        crop_and_save_image(train_path[num + i], box1, conf1, os.path.join(
            "../datasets/yolo_zz/{}/train/ori".format(all_attack_methods[0]),
            os.path.basename(train_path[num + i])))
        crop_and_save_image(train_path[i], box2, conf2, os.path.join(
            "../datasets/yolo_zz/{}/train/adv".format(all_attack_methods[0]),
    ...
    for i in range(num111):
        outputs_adv, outputs_post_adv, img_info_adv, outputs_conf_adv = predictor.inference(test_path[i])
        outputs_ori, outputs_post_ori, img_info_ori, outputs_conf_ori = predictor.inference(test_path[num111 + i])
        ...
        crop_and_save_image(test_path[num111 + i], box1, conf1, os.path.join(
            "../datasets/yolo_zz/{}/test/ori".format(all_attack_methods[0]),
            os.path.basename(test_path[num111 + i])))
        crop_and_save_image(test_path[i], box2, conf2, os.path.join(
            "../datasets/yolo_zz/{}/test/adv".format(all_attack_methods[0]),
    ...
    path111 = "../datasets/yolo_zz/{}/train".format(all_attack_methods[0])
    path222 = "../datasets/yolo_zz/{}/test".format(all_attack_methods[0])
    ...
    dataset = datasets.ImageFolder(
        root='../datasets/yolo_zz/{}/train'.format(all_attack_methods[0]),
        transform=transform)
    test_dataset = datasets.ImageFolder(
        root='../datasets/yolo_zz/{}/test'.format(all_attack_methods[0]),
        transform=transform)
    ...
    for epoch in range(train_epochs):
        ...
        if Test_Accuarcy > Best_Acc and Test_Accuarcy > 0.50:
            ...
            torch.save(model.state_dict(),
                       '../datasets/yolo_zz/{}_CNN_Classifier_{}.pth'.format(
                           all_attack_methods[0], Test_Accuarcy))
            saved_path = '../datasets/yolo_zz/{}_CNN_Classifier_{}.pth'.format(
                all_attack_methods[0], Test_Accuarcy)
...
def CNN_Classifier_Detect(params):
    ...
    for attack_method in all_attack_methods:
        # Initialize the dataset
        dataset = datasets.ImageFolder(
            root='../datasets/yolo_zz/{}/train'.format(attack_method),
            transform=transform)
        test_dataset = datasets.ImageFolder(
            root='../datasets/yolo_zz/{}/test'.format(attack_method),
            transform=transform)
        ...
        path333 = "../datasets/yolo_zz/{}/test".format(attack_method)
...
def detect_main(params):
    ...
    if args.device == "gpu":
        ...
        if args.fp16:
            model.half()  # to FP16
    model.eval()
    # 如果是中毒模型的话，模型权重路径改为以下：
    args.c = 'yolox_x.pth'
    if params['Attack_Methods'] == 'backdoor':
        args.c = 'best_ckpt.pth'

    if not args.trt:
        if args.ckpt is None:
            ckpt_file = os.path.join(file_name, "best_ckpt.pth")
            print('load best_ckpt.pth')
        else:
            ckpt_file = args.c
...
if __name__ == "__main__":
    params = {
        ...
        "Attack_Methods": ["ALL_Attack_warship"]  # {FGSM、IGSM、GuassianBlurAttack、PSO_patch
    }
