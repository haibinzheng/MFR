...
class MyVggNet_modelTransfer(nn.Module):
    def __init__(self, num_classes=10):
        super(MyVggNet_modelTransfer, self).__init__()
        # 预训练的vgg16特征提取层
        self.vgg = vgg
        # 添加新的全连接层
        self.classify = nn.Sequential(
...
num_classes = 10
epochs = 5
trained_model_name = 'S1_VGGNet_10Class_78Acc_4Epoch.pth' # 用于迁移学习的模型
transfer_train_data = 'train_transfer80_90class'
transfer_test_data = 'test_transfer80_90class'
...
model = MyVggNet_modelTransfer(num_classes=num_classes)
#导入预训练好的VGG模型
...
#            outputs,aux2,aux1 = model(inputs)
   ...
   if now_acc>acc1:
       ...
       torch.save(model.state_dict(), './VGGNet_models/S2_transferModel_VGGNet_%dClass_%dAcc_%dEpoch.pth'%(num_classes, acc1,epoch))