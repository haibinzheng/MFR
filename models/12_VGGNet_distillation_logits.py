...
trained_model_name = 'S9_transfer_VGGNet_10Class_80Acc_4Epoch.pth' # 用于蒸馏的模型
distillation_train_data = 'train_transfer10_20class' # 选择蒸馏的训练数据集
...
# 定义蒸馏温度T
T = 0.6 # 温度越高，从教师模型中蒸馏的信息越多，蒸馏温度取值【0，1】
...
optimizer=torch.optim.Adam(model_distilled.parameters(),lr=0.001)
# optimizer = optim.SGD(model_distilled.parameters(), lr=0.1, momentum=0.9, weight_decay=5e-4)
last=0
hostlist_set = []
acc1 = 0
for epoch in range(epochs):
   ...
   for i, data in enumerate(trainloader, 0):
       ...
       # print(labels.shape)
       teacher_label = model(inputs)
       # print(distill_label.shape, torch.argmax(teacher_label,dim=1).shape)
...
#            outputs,aux2,aux1 = model(inputs)
       ...
       if random.random()<T:
           loss = criterion(outputs, torch.argmax(teacher_label,dim=1))
       else:
       ...
       state = {'model': model_distilled.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
# get the ac with valdataset in each epoch
   ...
   with torch.no_grad():
       ...
       for i, data in enumerate(testloader, 0):
           ...
           outputs = model_distilled(images)
           _, predicted = torch.max(outputs.data, 1)
           total += labels.size(0)
   ...
   if now_acc>acc1:
       ...
       torch.save(model_distilled.state_dict(),
                  './VGGNet_models/S12_distillation_VGGNet_%dClass_%dAcc%dT.pth'%(num_classes, acc1, T*100))
...
#
# # 保存模型
# torch.save(model.state_dict(), './VGGNet_models/S12_distillation_VGGNet_%dClass_%dAcc_%dT.pth'%(num_classes, acc_tmp, T*100))
#
#
