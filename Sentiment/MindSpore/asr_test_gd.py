...
if __name__ == "__main__":
    FGSM_IMAGES_PATH = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/image_mindspore/dataset/PatchAttack_less_less"  # 测试数据集路径
    ...
    for file_path, root in all_files:
        ...
        with open(file_path, "rb") as img_file:
            ...
            #     ori_path = f'/data/Newdisk/chenjingwen/DT_B4/SJS_detect/Image_mindspore/dataset/PatchAttack_less/ori/{class_name}'
            #     if not os.path.exists(ori_path):  # 如果子文件夹不存在，创建它
            #         os.makedirs(ori_path)
            #     shutil.copy(file_path, os.path.join(ori_path, os.path.basename(file_path)))
            # if 'adv' in file_path.split('/') and class_pred != class_name:
            #     adv_path = f'/data/Newdisk/chenjingwen/DT_B4/SJS_detect/Image_mindspore/dataset/PatchAttack_less/adv/{class_name}'
            #     if not os.path.exists(adv_path):  # 如果子文件夹不存在，创建它
            #         os.makedirs(adv_path)
            #     shutil.copy(file_path, os.path.join(adv_path, os.path.basename(file_path)))
            if 'ori' in file_path.split('/') and class_pred == class_name:
                ori_correct += 1
