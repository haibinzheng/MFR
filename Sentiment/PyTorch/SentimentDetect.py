...
def SentimentDetect(params1):
    ...
    # ori_path = os.path.join('datasets', Dataset, 'test')
    # ori_path = 'datasets/gen_data/{}/{}/{}/png/ori'.format(Dataset, Model, Attack_Name)
    ori_path = '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/Image/datasets/gen_data/warship/VGG16/select_data/OnePixel/png/ori'

    model, testloader, testset = data_model_load_v1(Dataset, Model, ori_path, device)
    model_path = 'model_data/vgg16_ship.pth'

    model.load_state_dict(torch.load(model_path, map_location='cuda:0'))
    pickNumber_v1 = 270  # 控制训练二分类器所用原始样本数量
    ...
    adv_path = '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/Image/datasets/gen_data/warship/VGG16/select_data/OnePixel/png/adv'
...
def SentimentTrain(params1):
    ...
    path = '/data/Newdisk/chenjingwen/DT_B4/IndicatorsCoverage/SJS/Sentiment_backdoor/dataset/AlexNet/BadNets/ori'
    model, testloader, testset = data_model_load_v1(Dataset, Model, path, device)
    # model_path = 'model_data/{}_{}.pth'.format(Model, Dataset)
    model_path = '/data/Newdisk/chenjingwen/DT_B4/IndicatorsCoverage/SJS/Sentiment_backdoor/poi_model/BadNets/ResNet50_Military_Stealing_Model.pth'
    # model.load_state_dict(torch.load(model_path, map_location='cuda:0'))
    model.load_state_dict(torch.load(model_path, map_location={'cuda:6': 'cuda:7'}))
    pickNumber_v1 = 180 # 控制训练二分类器所用原始样本数量
    ...
    # adv_path = 'datasets/gen_data/{}/{}/{}/png/adv'.format(Dataset, Model, Attack_Name)
    adv_path = '/data/Newdisk/chenjingwen/DT_B4/IndicatorsCoverage/SJS/Sentiment_backdoor/dataset/AlexNet/BadNets/poi'
    ...
    # detector.load_state_dict(
    #     torch.load(
    #         "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/Image/model_data/detect/SentimentDetect_Military_VGG16_All-Attack.pth",
    #         map_location='cpu'))
    detector.to(device)
    criterion = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(detector.parameters(), lr=0.0001)
...
if __name__ == "__main__":
    ...
    # AttackMethodList = ['PatchAttack', 'OnePixel']
    AttackMethodList = ['BadNets']
    # AttackMethodList = ['FGSM-L1','MIFGSM-Lambda1','PGD-Lambda1','OnePixel','GaussianBlurAttack','ZOO-Lambda1']
