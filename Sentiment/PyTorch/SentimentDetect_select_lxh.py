...
# SentimentDetect
def SD_evaluate(classifier, detector, val_loader,true_labels,output_dir):
            ...
            for item in pred.cpu().numpy():
                if target == 0:
                    if item[0] == target:
                        print('检测成功！')
                        i += 1
                        input1 = inv_normalize(data.squeeze(0))
                        # print(input1)
                        ...
                        os.makedirs(output_dir + f'/png/ori/{true_labels[iii]}', exist_ok=True)
                        img.save(os.path.join(output_dir + f'/png/ori/{true_labels[iii]}/img_{i}.png'))
                        iii += 1
                    else:
                        if random.random() < 0:
                            i += 1
                    ...
                    if item[0] == target:
                        ...
                        array = (array * 255).astype(np.uint8)
                        img = Image.fromarray(array)
                        os.makedirs(output_dir + f'/png/adv/{true_labels[iii]}', exist_ok=True)
                        img.save(os.path.join(output_dir + f'/png/adv/{true_labels[iii]}/img_{ii}.png'))
                        iii += 1
...
# def SD_evaluate(classifier, detector, val_loader, true_labels, save_correct_dir):
#     detect_result = []
#     accs = AverageMeter()
#     classifier.eval()
#     detector.eval()
...
#                     img = data[n].cpu().numpy().transpose(1, 2, 0)  # Convert tensor to numpy array
#                     img = (img * 255).astype(np.uint8)  # Convert to 0-255 range for image saving
#                     img_pil = Image.fromarray(img)
#                     class_dir = os.path.join(save_correct_dir, f"{true_labels[i].item()}")
#                     if not os.path.exists(class_dir):
#                         os.makedirs(class_dir)
#                     img_pil.save(os.path.join(class_dir, f"{correct_count}.png"))
#
#             acc = correct.mul_(100.0 / data.size(0))
#
#             accs.update(acc.item(), data.size(0))
...
def SentimentDetect(params1):
    ...
    output_dir = f'/data/Newdisk/chenjingwen/DT_B4/SJS_detect/Image/datasets/gen_data/warship/{Model}/select_data/{Attack_Name}'
    ...
    pickNumber_v1 = 800  # 控制训练二分类器所用原始样本数量
   ...
   # adv_path = 'datasets/gen_data/{}/{}/select_data/{}/png/adv'.format(Dataset, Model, Attack_Name)
    ...
    print("Label (NumPy):", ture_label, ture_label_adv)
    ...
    detector.load_state_dict(
        torch.load(
            "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/Image/model_data/detect/SentimentDetect_warship_VGG16_All-Attack.pth",
            map_location='cpu'))

    #select_path = "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/Image/datasets/select_data/Military/GoogleNet/FGSM-Lambda1/"
    #Test_Accuarcy, detect_result = SD_evaluate(model, detector, test_dataloader, true_labels, select_path)
    Test_Accuarcy, detect_result = SD_evaluate(model, detector, test_dataloader, true_labels, output_dir)
    jsontext = {
