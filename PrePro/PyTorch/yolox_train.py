...
if __name__ == "__main__":
    ...
    FGSM_IMAGES_PATH = "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/YOLOX_adv_image_true/ALL"  # 测试数据集路径
    ...
    DETECTOR_PATH = '/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/Yolo_detect/save_model/ALL_v2_COCO_CNN_Classifier_0.8190212373037857.pth'
    exp = get_exp(None, 'yolox-x', )
    model = exp.get_model()
    model.to(device)
    ...
    for root, dirs, files in os.walk(FGSM_IMAGES_PATH):  # 以该数据集为例，可替换
        ...
        for file in files:
            if file.lower().endswith(supported_extensions):
                ...
                input = yolox_detection(file_path)
                ...
                # print("Mean (均值):", input.mean().item())  # 张量的均值
                # print("Sum (总和):", input.sum().item())  # 张量的元素总和
    ...
    for epoch in range(train_epochs):
        ...
        for image, label in zip(inputs, labels):
            image, label = image.to(device), torch.tensor([label], dtype=torch.long).to(device)

            optimizer.zero_grad()
            outputs = model(image)
            predicted_train = torch.argmax(outputs.data, dim=1)

            train_correct += (predicted_train == label).sum().item()
        ...
        with torch.no_grad():
            model.eval()
            for image111, label111 in zip(inputs, labels):
                image111, label111 = image111.to(device), torch.tensor([label], dtype=torch.long).to(device)
                output111 = model(image111)
                predicted_test = torch.argmax(output111.data, 1)
                test_correct += (predicted_test == label111).sum().item()
                total111 += 1
        ...
        if Test_Accuarcy > Best_Acc and Test_Accuarcy > 0.50:
            ...
            torch.save(model.state_dict(),
                       'save_model/{}_CNN_Classifier_{}.pth'.format('ALL_COCO', Test_Accuarcy))
