...
def train_binary_classifier(params):
    ssd = SSD()
    # 建立目录
    attack_methods = ['patch_attack']
    # for i in range(len(attack_methods)):
    #     os.makedirs(
    #         "Dataset_SSD/{}/train/adv".format(attack_methods[i]),
    #         exist_ok=True)
    #     os.makedirs(
    #         "Dataset_SSD/{}/test/adv".format(attack_methods[i]),
    #         exist_ok=True)
    #     os.makedirs(
    #         "Dataset_SSD/{}/train/ori".format(attack_methods[i]),
    #         exist_ok=True)
    #     os.makedirs(
    #         "Dataset_SSD/{}/test/ori".format(attack_methods[i]),
    #         exist_ok=True)
    #     os.makedirs(
    #         "DetectData/{}/ori".format(attack_methods[i]),
    #         exist_ok=True)
    #     os.makedirs(
    #         "DetectData/{}/adv".format(attack_methods[i]),
    #         exist_ok=True)
    ...
    for i in range(len(All_Attack_Methods)):
        ...
        if All_Attack_Methods[i] in ['PGD','backdoor']:
            adv_path = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/VOC_adv_image_truee/{}".format(
    ...
    ### 生成自制训练数据集
    All_Attack_Methods[0] = params['Attack_Method']
    ...
    for i in range(num):
        ...
        _, outputs_post_ori = ssd.detect_image(imageOri_pil)

        _, outputs_post_adv = ssd.detect_image(imageAdv_pil)
        ...
        # 图片裁剪和保存
        crop_and_save_image(train_path[num + i], box1, os.path.join(
            "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/ssd_ship/patch_attack/train/ori",
            os.path.basename(train_path[num + i])))
        crop_and_save_image(train_path[i], box2, os.path.join(
            "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/ssd_ship/patch_attack/train/adv" ,
    ...
    for i in range(num111):
        ...
        _, outputs_post_ori = ssd.detect_image(imageOri_pil_test)
        _, outputs_post_adv = ssd.detect_image(imageAdv_pil_test)
        ...
        box2 = outputs_post_adv[box_idx_adv][0:4]
        # 图片裁剪和保存
        crop_and_save_image(test_path[num111 + i], box1, os.path.join(
            "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/ssd_ship/patch_attack/test/ori",
            os.path.basename(test_path[num111 + i])))
        crop_and_save_image(test_path[i], box2, os.path.join(
            "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/ssd_ship/patch_attack/test/adv",
    ...
    path111 = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/ssd_ship/patch_attack/train"
    path222 = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/ssd_ship/patch_attack/test"
    ...
    #     root='Dataset_SSD/{}/train'.format(All_Attack_Methods[0]),
    #     transform=transform)
    # test_dataset = datasets.ImageFolder(
    #     root='Dataset_SSD/{}/test'.format(All_Attack_Methods[0]),
    #     transform=transform)
    #
    ...
    #                    'model_data/{}_CNN_Classifier_{}.pth'.format(
    #                        All_Attack_Methods[0], Test_Accuarcy))
    #         saved_path = 'model_data/{}_CNN_Classifier_{}.pth'.format(
    #             All_Attack_Methods[0], Test_Accuarcy)
    # logger.info(f"All_Attack_Methods={All_Attack_Methods[0]}")
    jsontext = {
        'Accuracy': Test_Accuarcy,
        "Attack_Method": All_Attack_Methods,
        'Save_path': saved_path if saved_flag else 'acc too low,not saved'
    }

    return jsontext


def CNN_Classifier_Detect(defparam):
    Attack_Method = defparam['Attack_Method']
    ssd = SSD()
    detect_result = []
    # 建立目录
    attack_methods = ['FGSM', 'IGSM', 'PGD', 'patch_attack', 'GuassianBlurAttack', 'backdoor']
    for i in range(len(attack_methods)):
        os.makedirs(
            "DetectData/{}/ori".format(attack_methods[i]),
            exist_ok=True)
    ...
    if Attack_Method in ['backdoor','PGD']:
        adv_path = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/VOC_adv_image_true/{}".format(
            Attack_Method)
    else:

        adv_path = "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/VOC_adv_image_true/{}".format(
            Attack_Method)
    ...
    ori_path = os.path.join(
        '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/VOCdevkit/VOC2007/JPEGImages')
    if os.path.isdir(ori_path):
        ori_file = get_image_list(ori_path)
    else:
        ori_file = [ori_path]
    ...
    for i in range(num222):
        ...
        _, outputs_post_ori = ssd.detect_image(imageOri_pil_detect)
        _, outputs_post_adv = ssd.detect_image(imageAdv_pil_detect)
        ...
        # 图片裁剪和保存
        crop_and_save_image(detect_path[num222 + i], box1, os.path.join(
            "DetectData/{}/ori".format(Attack_Method),
            os.path.basename(detect_path[num222 + i])))
        crop_and_save_image(detect_path[i], box2, os.path.join(
            "DetectData/{}/adv".format(Attack_Method),
    ...
    # Initialize the dataset
    test_dataset = datasets.ImageFolder(
        root='DetectData/{}'.format(Attack_Method),
        transform=transform)
    ...
    model = CNN().to(device)
    model_path = f'model_data/{Attack_Method}_CNN_Classifier.pth'
    # model_path = '/data0/BigPlatform/DT_Project/002-Code/002-Detect/FGSM_CNN_Classifier_0.9934640522875817.pth'
    # model_path = '/data0/BigPlatform/DT_Project/ssd-pytorch-master/backdoor_CNN_Classifier_0.9267015706806283.pth'
    # model_path = '/data0/BigPlatform/DT_Project/ssd-pytorch-master/GuassianBlurAttack_CNN_Classifier_0.9923857868020305.pth'
    # model_path = '/data0/BigPlatform/DT_Project/ssd-pytorch-master/MIFGSM_CNN_Classifier_1.0.pth'
    # model_path = '/data0/BigPlatform/DT_Project/ssd-pytorch-master/IGSM_CNN_Classifier_1.0.pth'
    # model_path = '/data0/BigPlatform/DT_Project/ssd-pytorch-master/FGSM_CNN_Classifier_0.9973821989528796.pth'
    # model_path = '/data0/BigPlatform/DT_Project/ssd-pytorch-master/patch_attack_CNN_Classifier_0.9651162790697675.pth'
    # model_path = '/data0/BigPlatform/DT_Project/ssd-pytorch-master/PGD_CNN_Classifier_0.9803370786516854.pth'
    ...
    path333 = "DetectData/{}".format(Attack_Method)
