...
from ship_utils.utils import (cvtColor, get_classes, get_new_img_size, resize_image,
...
def crop_and_save_image(image_path, box, save_path):
    ...
    # cropped_image = TF.crop(image, a1, a2, b1, b2)
    ### ssd的检测框是(y1,x1,y2,x2)
...
def train_binary_classifier(params):
    # 建立目录
    frcnn = FRCNN()
    attack_methods = ['FGSM', 'IGSM', 'PGD', 'PSO_patch', 'GuassianBlurAttack', 'backdoor']
    # image_SaveDir = "Dataset_FRCNN"
    ...
    All_Attack_Methods = [params['Attack_Method']]
    ...
    for i in range(len(All_Attack_Methods)):
        ...
        if All_Attack_Methods[i] in['PGD','backdoor']:
            adv_path = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/FILR_adv_true_mwb/{}".format(
    ...
    for i in range(num):
        ...
        print(f"***************1.训练数据检测*************************")
        print(f"***************1.1训练原始数据检测*************************")
        _, outputs_post_ori = frcnn.detect_image(imageOri_pil)
        print(f"***************1.2训练对抗数据检测*************************")
        _, outputs_post_adv = frcnn.detect_image(imageAdv_pil)
        ...
        crop_and_save_image(train_path[num + i], box1,
                            os.path.join("/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/frcnn_ship/detectData/patch_attack/train/ori",
                                         os.path.basename(train_path[num + i])))
        crop_and_save_image(train_path[i], box2,
                            os.path.join("/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/frcnn_ship/detectData/patch_attack/train/adv",
    ...
    for i in range(num111):
        ...
        print(f"***************1.测试数据检测*************************")
        print(f"***************1.1测试原始数据检测*************************")
        _, outputs_post_ori = frcnn.detect_image(imageOri_pil_test)
        print(f"***************1.2测试对抗数据检测*************************")
        _, outputs_post_adv = frcnn.detect_image(imageAdv_pil_test)
        ...
        crop_and_save_image(test_path[num111 + i], box1,
                            os.path.join("/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/frcnn_ship/detectData/patch_attack/test/ori",
                                         os.path.basename(test_path[num111 + i])))
        crop_and_save_image(test_path[i], box2,
                            os.path.join("/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/frcnn_ship/detectData/patch_attack/test/adv",
    ...
    path111 = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/frcnn_ship/detectData/patch_attack/train"
    path222 = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/frcnn_ship/detectData/patch_attack/test"
    ...
    # dataset = datasets.ImageFolder(root=image_SaveDir + '/{}/train'.format(All_Attack_Methods[0]), transform=transform)
    # test_dataset = datasets.ImageFolder(root=image_SaveDir + '/{}/test'.format(All_Attack_Methods[0]),
    #                                     transform=transform)
    #
...
def CNN_Classifier_Detect(defparam):
    ...
    if Attack_Method in ['PGD', 'backdoor']:
        adv_path = "/data/Newdisk/chenjingwen/DT_B4/GD_detect/object_detection/datasets/FILR_adv_true_mwb/{}".format(
            All_Attack_Methods[i])
    else:
        adv_path = "/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/FILR_adv_true_mwb/{}".format(
            All_Attack_Methods[i])
    ...
    ###读正常样本
    ori_path = '/data/Newdisk/chenjingwen/DT_B4/SJS_detect/object_detection/datasets/FILR_RGB_image_mwb/FGSM'
    if os.path.isdir(ori_path):
        ori_file = get_image_list(ori_path)
    else:
        ori_file = [ori_path]
    ...
    for i in range(num222):
        ...
        _, outputs_post_ori = frcnn.detect_image(imageOri_pil_detect)
        _, outputs_post_adv = frcnn.detect_image(imageAdv_pil_detect)
